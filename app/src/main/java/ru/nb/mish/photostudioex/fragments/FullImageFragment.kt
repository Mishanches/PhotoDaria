...
class FullImageFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Glide.with(this)
                .load(arguments?.getString(IntentHelper.PHOTO_URL)) // считываем строчку по параметру PHOTO_URL

                // если картинка заказчалась из сети, то метод не вызывается
                .listener(object : RequestListener<Drawable>{
                    override fun onResourceReady(resource: Drawable?, model: Any?, target: Target<Drawable>?, dataSource: DataSource?, isFirstResource: Boolean): Boolean {
                   return false
                    }

                    // если не скачалась картинка, нет интернета у пользователя, то картинка - из Drawable
    ...
    companion object {
        fun newInstance(imageUrl: String) : FullImageFragment {
            ...
            val args = Bundle()
            args.putString(IntentHelper.PHOTO_URL, imageUrl) // по ключу PHOTO_URL вставляем imageUrl
